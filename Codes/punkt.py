...
class PunktSentenceTokenizer(PunktBaseClass, TokenizerI):
    ...
    def _slices_from_text(self, text):
        ...
        # print("------> ",last_break, len(text.rstrip()))
    ...
    def _realign_boundaries(self, text, slices):
        ...
        Modified to realign boundaries when quotes and sentence ending char found.
        #Ananya
        """
        realign = 0
        # print(_pair_iter(slices))
        flag_p = False
        sl_flag = False
        m_flag = False
        in_flag = False
        p_text = ''
        # print([sl for sl in _pair_iter(slices)])
        iter_count = 0
        for sl1, sl2 in _pair_iter(slices):
            # print('\niteration : ', iter_count)
            # print('prev text : ', p_text)
            init_text = text[sl1]
            # print('init text : ', text[sl1])

            # print("sl1 : ", sl1)
            # print("sl2 : ", sl2)
            # print("realign before : ", realign)

            if(sl_flag==True and sl2!=None and realign==0):
                sl1 = sl3
                sl2 = sl4
                # print("sl_flag was True, hence ")
                # print("sl1 : ", sl1)
                # print("sl2 : ", sl2)

            else:
                # print("Else for sl_flag==True and sl2!=None, \nrealign = ", realign)
                # sl1 = slice(sl1.start+realign, sl1.stop)
                pass


            # sl1 = slice(sl1.start + realign, sl1.stop)
            if(realign!=0 and sl2!=None and sl_flag==False):
                sl1 = slice(sl1.start, sl2.stop)
                # print("when realign!=0 => ", sl1, realign) 
            else:
                sl1 = slice(sl1.start+realign, sl1.stop)
                # print("when realign==0 => ", sl1, realign) 
            # print("sl1 : ", sl1)
            ...
            if not sl2:
                ...
                if(iter_count!=0):
                    ...
                    if(m_flag == False):
                        ...
                        # print("remaining text :", text[sl1])
                        if(init_text.strip()!=text[sl1].strip()):
                            sl1 = slice(0, 0)
                        else:
                            sl2 = sl1 
                    else:
                    ...
                    if text[sl1]:
                        # print("text : ", text[sl1])
                        yield sl1
                        # print("yield sl1 = ", sl1, sl2)
                        # print('text finally : ',text[sl2])
                        # print("Now, ", sl_flag, realign)
                else:
                    yield sl1

                continue

            # print("Before, ", sl_flag, realign)
            # print("Text to search : ", text[sl2])
            # re_boundary_realignment = re.compile(r'["\')\]}]+?(?:\s+|(?=--)|$)', re.MULTILINE)
            ...
            if m:
                ...
                if(m.group(0).strip() in ['",', "',", ","]):
                    # print("Next sentence is a part of this piece.")
                    flag_p = True
                    # print("flag_p = True => ", sl1.start, sl2.start)
                    # print("Slicing : ",slice(sl1.start, sl2.stop))
                    # yield slice(sl1.start, sl2.stop)
                    # print("Slice1 : ", sl1)
                    realign = m.end()
                    # print("realign set = ", realign)
                    # print("text 1 : ",text[sl1])
                    p_text = text[sl1]
                    m_flag = False
                    

                elif(m.group(0).strip() in ['."','"',"'"]):
                    # print("Now decide here till where the text should be taken...")

                    flag_p = False

                    if(p_text.strip().endswith('!') or p_text.strip().endswith('.') or p_text.strip().endswith('?') or p_text.strip().endswith('?"') or p_text.strip().endswith('."') or p_text.strip().endswith('!"') or p_text.strip().endswith("!'") or p_text.strip().endswith("?'") or p_text.strip().endswith(".'")):
                        
                        q_text_match = re.search(r'"\s+"',text[sl2])
                        if(q_text_match!=None and q_text_match.start()==0):
                            # print("Two quotes in the beginning")
                            # print('Hehe.. you succeed breaking the sentence here. Dont Combine the next piece.')
                            realign = m.end()
                            in_flag = True

                        else:
                            # print("One quote, text continued...")
                            # print(text[sl2])
                            m2 = re.findall(r'\'|\?|\.|\!', text[sl2])
                            # print(m2)
                            ...
                            if(len(m2)>1):
                                ...
                                if(m2==['.']):
                                    # print('m2 continued ', m2)
                                    # print('text : ', text[sl2])
                                    m3 = re.findall(r'\'|\"', text[sl2])
                                    # print(m3)
                                    if(m3.count('"')>2 or m3.count("'")>2):
                                        realign = m.end()
                                        in_flag = True
                                    else:
                                        realign = len(text[sl2])
                        ...
                        # print("continue till end of text piece.")
                        realign = len(text[sl2])
                        
                    
                    # print('realign decided : ', realign)
                    # print('Added reach : ', sl2.start+realign)


                    sl1 = slice(sl1.start, sl2.start+realign)
                    sl2 = slice(sl2.start+realign, sl2.stop)
                    sl_flag = True
                    sl3 = sl1
                    sl4 = sl2
                    # print('slice sl3 : ', sl1, "sl4 : ",sl2)
                    # print("text 4 : ",text[sl1])
                    yield sl1
                    # yield sl2
                    p_text = text[sl1]
                    m_flag = True
                    ...
                    # print("Else check..")
                    # print("text 2 : ",text[sl1])
                    yield slice(sl1.start, sl2.start)
                    realign = m.end()
                    p_text = text[sl1]

            else:
                # print("In else--", sl1, sl2)

                if(text[sl2].strip() in ["'.", '".', "!", ".", "?"]):
                    # print("Text is piece of previous sentence. incorrect sentence structure.")
                    sl1 = slice(sl1.start, sl2.stop)
                    # print("text 3 : ",text[sl1])
                    p_text = text[sl1]
                    # print("Else Sl1 ", sl1)
                    yield sl1
                    # print("Last : ",sl1, sl2)
                    sl_flag=False
                    realign = len(text[sl2].strip())

                else:
                    realign = 0
                    if text[sl1] and sl1.stop!=sl2.stop and sl_flag==False:
                        # print("text 3 : ",text[sl1])
                        p_text = text[sl1]
                        # print("Else Sl1 ", sl1)
                        yield slice(sl1.start, sl1.stop)
                        # print("Last : ",sl1, sl2)

                    else:
                        # print("Else within Else => ",realign)
                        sl_flag = False
                        # print("m_flag = ", m_flag)
                        if(m_flag == True and in_flag==True):
                            # print("Need to Handle this -> ", text[sl1])
                            # print('text : ', text[sl1])
